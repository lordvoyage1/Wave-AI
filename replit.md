...
- **Home is no longer lazy-loaded** — the heaviest page (`Home.tsx`) is now bundled eagerly into the main entry chunk. Previously it was code-split with `React.lazy()` and shown behind a `Suspense` fallback, which caused the "infinite loading spinner" symptom users saw after signup or after clicking "Continue as guest" on slow networks or with stale service-worker caches. The main bundle grew from 56 KB → 123 KB (35 KB gzipped) — still well within budget — and `/app` now renders synchronously with no Suspense at all. Vendor chunks (react, supabase, ui-vendor) remain split for caching.
- **PageSpinner stuck-recovery** — for the secondary lazy routes (Profile, About, Terms, ApiDocs, NotFound) the `PageSpinner` now waits 6 seconds; if the chunk still hasn't resolved it shows a "Reload" button that unregisters any service worker and hard-refreshes the page, so users can never get permanently stuck on a spinner.
- **Service worker cache bumped to `wave-ai-v6`** — forces clients with old cached chunks (from the previous deploy) to evict them on the next visit, preventing "old index.html references missing chunk hash" failures.
- **Guest button removed from Landing** — the "Continue as guest" entry point now lives ONLY on `/login` and `/signup`, not on `/`. The Landing page is back to a single primary CTA ("Chat With Me Now" → /login).
